...
## Приоритет после проверки по критериям

Работа ограничена обязательным минимумом, стабильностью, воспроизводимостью
и демонстрацией ценности. Проверены backend/frontend, STT, diarization, alignment,
извлечение, summary, экспорт, тесты и README. Исправлены потеря условия/сроков
в кратком списке экспортируемого протокола и недостаточная проверка полей по
показанным цитатам. Архитектура сохранена; **213 тестов прошли**, включая браузер.

Главный следующий приоритет — подготовить отсутствующие Community-1/Ollama и
проверить настоящий RU/KZ сценарий. Дополнительные интеграции не начаты.
Матрица официальных критериев и план показа: [JUDGING_REVIEW.md](JUDGING_REVIEW.md).

## Обновление: этапы 15–16 — тестирование и анализ расхода

Оба протокола из приложений Б/В перенесены в fixtures без готовых summary/таблиц:
32 и 28 реплик. 17 новых unit tests с mocked LocalLLMClient покрывают все девять
запрошенных правил и ошибки в источниках; сеть в этих тестах запрещена.
Настоящая модель запускается только отдельным manual-скриптом с явным флагом.

Добавлены локальные JSON-метрики: объём транскрипта, оценка input tokens, модель,
duration, cache hit/miss и текущие LLM calls. Полный вход — 1 вызов, кеш/GET — 0;
ошибки экспорта, рестарт и параллельные операции проверены. Текст в метрики не пишется.
Итог: **205 passed, 2 skipped** (опциональный браузер); качество реальной LLM
не подтверждается mock-тестами. Подробности: [TESTING_AND_METRICS.md](TESTING_AND_METRICS.md).

## Обновление: этапы 12–14 — API, экспорт, UI

Добавлены `/analyze` и read-only `/result`, общая схема результата и необязательная
дата встречи в отдельной таблице без изменения старых строк. DOCX/PDF используют
единый шаблон с нужными заголовками и таблицей поручений. Простой frontend на том же
FastAPI покрывает загрузку → обработку → progress → вкладки → скачивание.
Вкладки, перезагрузка страницы и чтение результата не вызывают AI.

Сверены F-07/F-08/F-09, файловый сценарий и локальная обработка источника требований.
Старые сервисы и endpoints сохранены. Реальные ML-веса/качество по-прежнему требуют
подготовки и проверки. Команды и границы проверок: [UI.md](UI.md).
Проверки: **178 passed**, включая два Chrome-сценария; `/health` — 200,
compileall без ошибок. UI/PDF просмотрены визуально, DOCX проверен структурно.
Исторические отчёты ниже описывают состояние на соответствующем этапе.

## Обновление: этапы 10–11 — экономный анализ и pipeline

На основе существующих сервисов добавлен `/process`: STT → diarization →
alignment → LocalMeetingAnalyzer → validation → SQLite → DOCX/PDF. Короткая
запись получает поручения и summary одним inference. Только после явного
переполнения контекста части возвращают компактные facts/action_items, затем
выполняется один merge; отдельные chunk-summary не генерируются. Лимиты запросов
и частей ограничены. Готовые результаты переиспользуются, ошибка экспорта не
повторяет LLM. Старые endpoints и таблицы сохранены.

170 тестов прошли; HTTP health — 200. Сквозной тест использует реальные ffmpeg,
SQLite и DOCX/PDF с читаемыми RU/KZ текстами, ML — явные doubles. Реальное
качество всех моделей и визуальная приёмка протоколов не подтверждены.
Состояние и команды: [PIPELINE.md](PIPELINE.md). Исторические отчёты ниже сохраняются.
...
| Привязка речи и поручений к конкретным участникам, F-06 | P0 | Alignment, сопоставление участников, анализ | Добавлен консервативный alignment сегментов с ручными именами; неоднозначность сохраняется |
| Транскрипт и структурированный список поручений, F-09 | P0 | Схемы, API, UI, SQLite | Сохранённый /result и UI с источниками готовы; настоящее LLM-качество не проверено |
...
| Экспорт протокола, F-07 | P0 | python-docx, ReportLab | Шаблон и таблицы DOCX/PDF готовы; RU/KZ, длинная строка и PDF визуально проверены; DOCX проверен структурно |
